...
ISO_NAME="tahoe-linux-amd64.iso"
...
lb config \
  ...
  --image-name tahoe-linux \
  --iso-application "Tahoe Linux" \
  --iso-publisher "Tahoe Project" \
...
if [[ ! -f tahoe-linux-amd64.hybrid.iso ]]; then
  echo "Build completed but expected ISO not found."
  exit 1
fi

cp -f tahoe-linux-amd64.hybrid.iso "$DIST_DIR/$ISO_NAME"
